...
public class StateAgendamento extends State {
	...
	JComboBox cbxProfissional = new JComboBox<ProfSaude>(
			ControllerAgendamento.getAllProf());
	...
	JComboBox<Paciente> cbxPac = new JComboBox<Paciente>(
			ControllerAgendamento.getAllPac());
	MeuSpiner tmIn = new MeuSpiner(), tmOut = new MeuSpiner();
	JButton btnSalvar = new JButton("Salvar"), btnDesmarcar = new JButton(
			"Desmarcar"), btnMarcar = new JButton("Marcar"),
	...
	protected StateAgendamento(JanelaPrincipal janela_) {
		...
		// myp
		myPainel.setBorder(new MatteBorder(0, 0, 0, 1, (Color) new Color(0, 0,
				0)));
		...
		lblEscolhaUmProfissional.setFont(new Font("Segoe UI Semibold",
				Font.PLAIN, 14));
		lblEscolhaUmProfissional.setHorizontalAlignment(SwingConstants.LEFT);
		lblEscolhaUmProfissional.setBounds(10, 28, 148, 20);

		cbxProfissional.addFocusListener(ControllerAgendamento
				.cbxExpand(cbxProfissional));
		cbxProfissional.addActionListener(ControllerAgendamento.cbxChange(
				cbxProfissional, pnlCalendar));
		cbxProfissional.setBounds(10, 54, 377, 20);

		cbxProfissional.setSelectedIndex(0);

		pnlCalendar.getCalendarTable().addMouseListener(
				ControllerAgendamento.calendarSel(pnlCalendar));
		...
		lblConsultasMaracadasPara.setFont(new Font("Segoe UI Semibold",
				Font.PLAIN, 14));

		lstConsultas.addListSelectionListener(ControllerAgendamento
				.mudouLista());
		...
		btnMarcarNovaConsulta.addActionListener(ControllerAgendamento
				.btnNovo(pnlCalendar));
	...
	public void updateA3(Consulta c, String prof, String Esp, Paciente pac,
			Comparable inicio, Comparable fim, Comparable cinicio,
			Comparable cfim, boolean ehNovo) {
