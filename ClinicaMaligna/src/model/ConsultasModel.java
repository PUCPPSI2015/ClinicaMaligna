...
public class ConsultasModel extends Model {
	...
	public static void listaRefresh() {
		...
		try {
			...
			while (rstExiste.next()) {
				consultas.add(new Consulta(rstExiste.getInt("Id"), rstExiste
						.getInt("IdPaciente"), rstExiste
						.getInt("IdDisponibilidade"), rstExiste
						.getString("Observacoes"), rstExiste
						.getString("PedidosDeExame"), rstExiste
						.getString("Prescricoes"), rstExiste
						.getString("Recomendacoes"), rstExiste
						.getTime("Inicio"), rstExiste.getTime("Fim"), rstExiste
						.getDate("Data"), rstExiste.getInt("ativo")));
			}
	...
	public static Consulta[] getAtivasPassadasByProf(int id) {
		java.util.Date hoje = formatarCompleta(uf.format(cal.getTime())); // montar
																			// data
																			// de
																			// hoje
																			// com
		...
		listaRefresh();
		ArrayList<Consulta> estasconsultas = new ArrayList<Consulta>();
		for (int key = 0; key < consultas.size(); key++) { // procurar em todas
															// as consultas
			Consulta esta = consultas.get(key);
			...
			int idprof = DisponibilidadesModel.getById(
					esta.getIdDisponibilidade()).getIdProfissional();

			if (esta.getAtivoBool() && (idprof == id)
					&& (daConsulta.before(hoje))) { // se for ativa, e id do
													// profissional procurado e
													// for antes da data de hoje
	...
	public static Consulta[] getAtivasFuturasByProf(int id) {
		...
		for (int key = 0; key < consultas.size(); key++) { // procurar em todas
			...
			if (esta.getAtivoBool() && (idprof == id)
					&& (daConsulta.after(hoje) || daConsulta.equals(hoje))) { // se
																				// for
																				// ativa,
																				// e
																				// id
	...
	public static void updateConsultaParcial(int id, Date d, Time i, Time f,
			int p, int dd) {

		try {
			myStm.executeUpdate("update " + tabelanome + " " + "set Data='" + d
					+ "', Inicio='" + i + "', Fim='" + f + "', IdPaciente='"
					+ p + "', " + "IdDisponibilidade='" + dd + "' "
					+ "where Id=" + id + " ");

		} catch (SQLException e) {
			ControllerPrincipal.gritar("Erro de gravação no banco de dados",
					"Algo de errado não está certo");
			e.printStackTrace();
		}
	}

	public static void updateConsultaMeta(int id, String observacoes,
			String Exames, String prescri, String recomendacoes) {

		try {
			myStm.executeUpdate("update " + tabelanome + " "
					+ "set Observacoes='" + observacoes + "', PedidosDeExame='"
					+ Exames + "', Prescricoes='" + prescri
					+ "', Recomendacoes='" + recomendacoes + "' " + "where Id="
					+ id + " ");
	...
	public static void excluirConsulta(int id) {
		String deletacao = "delete from " + tabelanome + " " + "where Id=" + id
				+ "";
		try {
			myStm.executeUpdate(deletacao);
		} catch (SQLException e) {
			ControllerPrincipal.gritar("Erro de exclusao no banco de dados",
					"Algo de errado não está certo");
			e.printStackTrace();
		}
	}
	...
	public static void desmarcarConsulta(int id) {
		...
		try {
			myStm.executeUpdate("update " + tabelanome + " " + "set ativo=0 "
					+ "where Id=" + id + " ");
	...
	public static void marcarConsulta(int id) {
		...
		try {
			myStm.executeUpdate("update " + tabelanome + " " + "set ativo=1 "
					+ "where Id=" + id + " ");
	...
	public static Consulta[] getPesquisaPac(String pesquisa) {
		Paciente[] paciente = PacientesModel.getPesquisa(pesquisa);// pegar
																	// pacientes
																	// coma
																	// pesquisa
	...
	public static class Consulta {
		...
		public Consulta(int id, int idPaciente, int idDisponibilidade,
				Time inicio, Time fim, Date data, int ativo) {
			this(id, idPaciente, idDisponibilidade, inicio, fim, data);
			this.ativo = ativo;
		}

		public Consulta(int id, int idPaciente, int idDisponibilidade,
				Time inicio, Time fim, Date data, boolean ativo) {
			this(id, idPaciente, idDisponibilidade, inicio, fim, data);
			if (ativo)
				this.ativo = 1;
			else
		...
		public Consulta(int id, int idPaciente, int idDisponibilidade,
				String observacoes, String pedidosExame, String prescricoes,
				String recomendacoes, Time inicio, Time fim, Date data) {
			this(id, idPaciente, idDisponibilidade, inicio, fim, data);
		...
		public Consulta(int id, int idPaciente, int idDisponibilidade,
				...
				String recomendacoes, Time inicio, Time fim, Date data,
				int ativo) {
			this(id, idPaciente, idDisponibilidade, observacoes, pedidosExame,
					prescricoes, recomendacoes, inicio, fim, data);
			this.ativo = ativo;
		}
		...
		public String toString() {
			...
			return "Consulta "
					+ aStr
					+ " "
					+ PacientesModel.getById(idPaciente).toString()
					+ " : "
					+ Especialidades.getOne(
							DisponibilidadesModel.getById(idDisponibilidade)
									.getIdEspecialidade()).toString();
		}
