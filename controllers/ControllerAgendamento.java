...
public class ControllerAgendamento {
	...
	public static ActionListener cbxChange(JComboBox cbx, Calendario c){
		return new ActionListener () {
			public void actionPerformed(ActionEvent arg0) {
				...
				dom = DisponibilidadesModel.getByProfAndDia(selectedProf.getId(), 1);
				seg = DisponibilidadesModel.getByProfAndDia(selectedProf.getId(), 2);
				ter = DisponibilidadesModel.getByProfAndDia(selectedProf.getId(), 3);
				qua = DisponibilidadesModel.getByProfAndDia(selectedProf.getId(), 4);
				qui = DisponibilidadesModel.getByProfAndDia(selectedProf.getId(), 5);
				sex = DisponibilidadesModel.getByProfAndDia(selectedProf.getId(), 6);
				sab = DisponibilidadesModel.getByProfAndDia(selectedProf.getId(), 7);
		...
		return new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ...
                if (row >= 0 && col >= 0) { //verificar se é uma data válida 
                	...
                	selectedDisp = DisponibilidadesModel.getByProfAndDia(selectedProf.getId(), col+1);
                	//pegar consultas com essa disponibilidade
                	consultas = ConsultasModel.getByDispoData(selectedDisp.getId(), selectedData);
	...
	public static void updateA3(Consulta c){
		...
		state.updateA3(c, p.getNome(), e.getNome(), pac, d.getInicio(), d.getFim(), c.getInicio(), c.getFim(), false);
	}
	...
	private static class ExcluirAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			int resposta = ControllerPrincipal.perguntar("Deseja mesmo excluir permanentemente esta consulta?", "Certeza?");
			if(resposta == 0){
				state.fillConsultas(consultas);
				state.updateA3();
			}
	...
	private static class MarcarAction implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if((c.getInicio().before(selectedDisp.getInicio())) || (c.getFim().after(selectedDisp.getFim()))){
				ControllerPrincipal.gritar("O horario excede a disponibilidade deste profissional","Erro no horário");
				return;
			}
			
			//validar com cponsultas
			Consulta[] conflitantes = ConsultasModel.getByDispoData(c.getIdDisponibilidade(), c.getData());
			for(int h = 0; h < conflitantes.length; h++){
				
				Consulta comparada =  conflitantes[h];
				if((c.getId() != comparada.getId()) && (comparada.getAtivoBool())){
					if(c.getInicio().after(comparada.getInicio())){
						if(comparada.getFim().after(c.getInicio())){
							ControllerPrincipal.gritar("O horario inserido entra em comflito com outras consultas marcadas para este dia, insira outro horario","Erro no horário");
							return;
						}
					}else{
	...
	private static class NovoAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent ev) {
			if((selectedData == null)||(selectedProf == null) || (selectedDisp == null)){
				ControllerPrincipal.gritar("Selecione um profissional e uma data no painel a esquerda.");
				return;
			}
			Consulta nova = new Consulta(-1, 0, selectedDisp.getId(), selectedDisp.getInicio(), selectedDisp.getFim(), selectedData);
			Especialidade e = Especialidades.getOne(selectedDisp.getIdEspecialidade());
			ProfSaude p = ProfSaudeModel.getOne(selectedDisp.getIdProfissional());
			Paciente pac = PacientesModel.getById(nova.getIdPaciente());

			state.updateA3(nova, p.getNome(), e.getNome(), pac, selectedDisp.getInicio(), selectedDisp.getFim(), nova.getInicio(), nova.getFim(), true);
		}
	...
	private static class SalvarAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if((i.before(selectedDisp.getInicio())) || (f.after(selectedDisp.getFim()))){
				ControllerPrincipal.gritar("O horario excede a disponibilidade deste profissional","Erro no horário");
				return;
			}
			...
			if(c.getId() != -1){
				...
				ConsultasModel.novaConsultaParcial(c.getData(), i, f, paciente, c.getIdDisponibilidade());
			}
