...
public class ControllerRegistro {
	...
	public static Consulta[] getPassadas(){
		return ConsultasModel.getAtivasPassadasByProf(((ProfSaude) ControllerPrincipal.getObjLogado()).getId());
	}
	public static Consulta[] getFuturas(){
		return ConsultasModel.getAtivasFuturasByProf(((ProfSaude) ControllerPrincipal.getObjLogado()).getId());
	}
	...
	public static void updateFriedman(Consulta c){
		...
		state.updateFriedman(c, pac.getNome(), e.getNome(), c.getData(), c.getInicio(), c.getFim(), c.getObservacoes(), c.getPedidosExame(), c.getPrescricoes(), c.getRecomendacoes());
	}
	...
	private static class Salvar implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(obs.length() > 1024) obs = obs.substring(0, 1024); 
			if(exm.length() > 1024) exm = exm.substring(0, 1024); 
			if(pre.length() > 1024) pre = pre.substring(0, 1024); 
			if(rec.length() > 1024) rec = rec.substring(0, 1024); 
