	...
	public static void preencher() {
		...
		try {
			...
			while (rstCargos.next()) {
				especialidades.put(rstCargos.getInt("Id"), new Especialidade(
						rstCargos.getInt("Id"), rstCargos.getString("Nome"),
						rstCargos.getInt("IdProfissao")));
			}
	...
	public static Especialidade[] getThisProf(int prof) {
		...
		for (int key : especialidades.keySet()) {
			...
			if (esta.getIdProf() == prof)
				estasEspecialidades.put(
						esta.getId(),
						new Especialidade(esta.getId(), esta.getNome(), esta
								.getIdProf()));
		}
